...
export default class PdfEditorView extends ScrollView {
  ...
  updatePdf(closeOnError = false) {
    ...
    PDFJS.getDocument(pdfData).then((pdfDocument) => {
      this.container.find(".page-container").remove();
      this.pageContainers = [];
      this.canvases = [];
      this.textLayers = [];
      this.pageHeights = [];
      ...
      for (let pdfPageNumber of _.range(1, this.pdfDocument.numPages+1)) {
        let pageContainer = $("<div/>", {class: "page-container"}).appendTo(this.container)[0];
        let canvas = $("<canvas/>", {class: "page-canvas"}).appendTo(pageContainer)[0];
        let textLayer = $("<div/>", {class: "page-text textLayer"}).appendTo(pageContainer)[0];
  ...
  renderPdf(scrollAfterRender = true) {
    ...
    Promise.all(
      _.range(1, this.pdfDocument.numPages + 1).map((pdfPageNumber) => {
        let pageContainer = this.pageContainers[pdfPageNumber - 1];
        let canvas = this.canvases[pdfPageNumber - 1];
        let textLayer = this.textLayers[pdfPageNumber - 1];
        ...
        return this.pdfDocument.getPage(pdfPageNumber).then((pdfPage) => {
          ...
          this.pageHeights[pdfPageNumber - 1] = viewport.height;

          let textRenderTask = pdfPage.getTextContent().then(textContent => {
          ...
          let canvasRenderTask = pdfPage.render({canvasContext: context, viewport: viewport});

          return Promise.all([textRenderTask, canvasRenderTask])
        });
      })
    ).then((renderTasks) => {
